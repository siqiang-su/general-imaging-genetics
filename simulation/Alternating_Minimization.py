  ...
  for i in range(len(lam1_grid)):
     for j in range(len(lam2_grid)):
        for k in range(len(R_grid)):
            a,b,thetahat = cca_alt_minimization(N=len(z),X=X,Yarray=Yarray,z=z,lam1=lam1_grid[i],lam2=lam2_grid[j],R=R_grid[k],tau_x=10,tau_y=0.1,p1=p1,p2=p2,d1=d1,d2=d2,p=p,niter=niter)
